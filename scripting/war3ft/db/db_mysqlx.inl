...
new const szTables[TOTAL_TABLES][] = 
{
	...
	//Äëÿ talisman +
	"CREATE TABLE IF NOT EXISTS `wc3_player_talisman` ( `player_id` int(8) unsigned NOT NULL default '0', `talisman_id` tinyint(4) unsigned NOT NULL default '0', PRIMARY KEY  (`player_id`,`talisman_id`) ) ENGINE=MyISAM;",
	"CREATE TABLE IF NOT EXISTS `wc3_num_talisman` ( `player_id` int(8) unsigned NOT NULL default '0', `talisman_num` tinyint(4) unsigned NOT NULL default '0', PRIMARY KEY  (`player_id`,`talisman_num`) ) ENGINE=MyISAM;"
...
MYSQLX_FetchUniqueID( idUser )
{
	...
	if ( SQL_NumResults( query ) == 0 )
	{
		...
		get_user_name( idUser, szName, 69 );
		DB_FormatString( szName, 69 );
		get_user_ip( idUser, szIP, 19, 1 );
		get_user_authid( idUser, szSteamID, 29 );

		format( szQuery, 511, "INSERT INTO `wc3_player_extra` ( `player_id` , `player_steamid` , `player_ip` , `player_name` ) VALUES ( '%d', '%s', '%s', '%s' );", g_iDBPlayerUniqueID[idUser], szSteamID, szIP, szName );
...
MYSQLX_Save( idUser )
{
	...
	{
		new szName[128];
		get_user_name( idUser, szName, 127 );
	...
	if ( !SQL_Execute( query ) )
	{
		format(szMessage, sizeof(szMessage) - 1, "^x01%L^x03%L^x01%L ^x04%L",
				...
				LANG_PLAYER,"CLIENT_PRINT_MYSQL_ERR_SAVE");	
	...
	if ( p_data[idUser][P_RACE] != RACE_CHAMELEON )
	{
		...
		for ( new iSkillID = 0; iSkillID < MAX_SKILLS; iSkillID++ )
		{
			...
			{
				iCurrentLevel = SM_GetSkillLevel( idUser, iSkillID, 14 );
...
MYSQLX_Save_T( idUser )
{
	...
	if ( p_data[idUser][P_RACE] != RACE_CHAMELEON )
	{
		...
		for ( new iSkillID = 0; iSkillID < MAX_SKILLS; iSkillID++ )
		{
			if ( g_SkillType[iSkillID] != SKILL_TYPE_PASSIVE )
			{
				iCurrentLevel = SM_GetSkillLevel( idUser, iSkillID, 15 );
...
MYSQLX_GetAllXP(idUser)
{
	...
	if ( !SQL_Execute( query ) )
	{
		format(szMessage, sizeof(szMessage) - 1, "^x01%L^x03%L^x01%L ^x04%L",
				LANG_PLAYER,"WC3_BRACKET_LEFT",LANG_PLAYER,"WC3_MODCLIENT",LANG_PLAYER,"WC3_BRACKET_RIGHT",
				LANG_PLAYER,"CLIENT_PRINT_MYSQL_ERR_UN_RET_XP");	
...
MYSQLX_Error( Handle:query, szQuery[], idUser )
{
	...
	WC3_Log( true, "[MYSQLX] Error in querying database, location: %d", idUser);
...
MYSQLX_ThreadError( Handle:query, szQuery[], szError[], iErrNum, failstate, idUser )
{
	WC3_Log( true, "[MYSQLX] Threaded query error, location: %d", idUser );
	WC3_Log( true, "[MYSQLX] Message: %s (%d)", szError, iErrNum );
	WC3_Log( true, "[MYSQLX] Query statement: %s ", szQuery );
...
MYSQLX_UpdateTimestamp( idUser)
{
	...
	new szQuery[256];
	format( szQuery, 255, "UPDATE `wc3_player` SET time = NOW() WHERE ( `player_id` = '%d' );", DB_GetUniqueID( idUser ) );
