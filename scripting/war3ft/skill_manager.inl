...
SM_SetPlayerRace( idUser, iRace )
{
	...
	if ( iRace == RACE_CHAMELEON )
	{
		...
		if (get_pcvar_num( CVAR_wc3_vip_chameleon ) == 1 )
...
SM_GetSkillLevel( idUser, skill_id, debug_id = -1 )
{
...
SM_SetSkill( idUser, iSkillID )
{
	...
	{
		WC3_Log( false, "[41] Invalid skill: %d", iSkillID );

		log_error( AMX_ERR_NATIVE, "[41] Invalid skill: %d", iSkillID );

		return;
	}

	// Get the user's current skill level
	new iCurrentLevel = SM_GetSkillLevel( idUser, iSkillID, 10 );
...
SM_GiveRandomSkillPoint( idUser )
{
	...
	if ( SM_TotalSkillPointsUsed( idUser ) >= p_data[idUser][P_LEVEL] )
	{
		return false;
	}
	...
	new iSkillLevel = SM_GetSkillLevel( idUser, iRandomSkill, 12 );

	// Sweetest conditional statement ever
	while ( iSkillLevel + 1 > MAX_SKILL_LEVEL || p_data[idUser][P_LEVEL] <= 2 * iSkillLevel )
	{
		//server_print( "[%d:%d] %d >= %d || %d <= %d", iRandomSkill, iSkillLevel, iSkillLevel + 1, MAX_SKILL_LEVEL, p_data[idUser][P_LEVEL], 2 * iSkillLevel );

		iRandomSkill = SM_GetRandomSkillByType( idUser, SKILL_TYPE_TRAINABLE );
		iSkillLevel = SM_GetSkillLevel( idUser, iRandomSkill, 13 );
	}
			
	// Set up the skill...
	SM_SetSkill( idUser, iRandomSkill );

	//client_print( idUser, print_chat, "[DEBUG] (%d) Trainable given - from %d to %d", iRandomSkill, iSkillLevel, iSkillLevel + 1 );

	return true;
}

// After a user's skill has changed - the skill is configured here!
//SM_SkillSet( idUser, iSkillID, iPreviousSkillLevel = 0, iNewSkillLevel = 0 )
