...
{
	if ( get_pcvar_num( CVAR_wc3_races ) == 10 && get_pcvar_num( CVAR_wc3_cham_random ) )
	{
		...
		new iPlayers[32], iNumPlayers, idUser;
		...
		for ( i = 0; i < iNumPlayers; i++ )
		{
			...
			if ( p_data[idUser][P_RACE] == RACE_CHAMELEON )
			{
				...
				SM_SetPlayerRace( idUser, RACE_CHAMELEON );
...
CHAM_Configure()
{
	//if (get_pcvar_num( CVAR_wc3_vip_chameleon ) != 0 )
	//	return;
...
{	
	...
	while ( SM_GiveRandomSkillPoint( idUser ) ) {}
